...
this.chem.data <- subset(this.chem.data, !is.na(as.numeric(Area)))
      ...
      this.chem.data$Analysis.Parameters<-""
    this.chem.data$Level0.Sheet <- "Data"
    this.chem.data$Lab.Compound.Name <- this.chem
...
# Get rid of column header rows:t
  ...
  this.chem.data$Type <- this.chem.data$Lab.Sample.Name
  this.chem.data$Note <- ""
  this.chem.data$Cal <- this.chem.data$Date

  this.chem.data$Analysis.Method <- "GCMS"
  this.chem.data$Analysis.Instrument <- "Agilent 7890 GC w 7010 triple quadrupole"
  this.chem.data$Level0.File <- this.file

  UC.data <- rbind(UC.data,this.chem.data[,colnames(UC.data)])  
  dim(UC.data)
```
...
```{r UC_276_467_Athens_Mix 2a and b20220913, eval=TRUE}
  ...
  this.file.data <- NULL
  chem.names <- c("276","467")
  chem.cols <- c("...9", "...10")
  istd.col <- "...5"
  conc.col <- "nm after crash"
  sample.name.col <-"...2"
  for (this.chem.index in 1:2)
  {
...
level2[ccobs & zeroobs,"Verified"] <-"JFW: Excluded cal curve no signal"
