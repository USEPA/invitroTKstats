...
dat <- as.data.frame(read_excel("20200402_PFAS_UC_PFOA_PFOS.xlsx",sheet=3,skip=12))
dat[is.na(dat$Type),"Type"] <- ""
colnames(dat)[colnames(dat)=="IS Area"] <- "IS.Area"
colnames(dat)[colnames(dat)=="Sample Text"] <- "Sample.Text"
...
dat$Std..Conc <- dat$Std..Conc / 414.07
colnames(dat)[11] <- "uM"
...
# read from the Excel file using library(gdata)
dat <- as.data.frame(read_excel("20200402_PFAS_UC_PFOA_PFOS.xlsx",sheet=4,skip=11))
dat[is.na(dat$Type),"Type"] <- ""
colnames(dat)[colnames(dat)=="IS Area"] <- "IS.Area"
colnames(dat)[colnames(dat)=="Sample Text"] <- "Sample.Text"
...
dat$Std..Conc <- dat$Std..Conc / 305 * 414.07
colnames(dat)[11] <- "uM"
# Save the data:
...
# read from the Excel file using library(gdata)
dat <- as.data.frame(read_excel("20200402_PFAS_UC_PFOA_PFOS.xlsx",sheet=5,skip=11))
dat[is.na(dat$Type),"Type"] <- ""
colnames(dat)[colnames(dat)=="IS Area"] <- "IS.Area"
colnames(dat)[colnames(dat)=="Sample Text"] <- "Sample.Text"
...
dat <- subset(dat,!is.na(Response))
# adjust to actual molecular weight:
dat$Std..Conc <- dat$Std..Conc / 305 * 500.13 
# Save the data:
...
# read from the Excel file using library(gdata)
dat <- as.data.frame(read_excel("20200402_PFAS_UC_PFOA_PFOS.xlsx",sheet=6,skip=11))
dat[is.na(dat$Type),"Type"] <- ""
colnames(dat)[colnames(dat)=="IS Area"] <- "IS.Area"
colnames(dat)[colnames(dat)=="Sample Text"] <- "Sample.Text"
