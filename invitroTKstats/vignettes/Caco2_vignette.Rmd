...
Therefore, we replace the NaNs with 0 so that the "Blank" samples types are kept in the datset and our estimates are accurate. 

```{r}
# Response = Peak.Area/ISTD.Area --> NaN when ISTD.Area = 0 
# Replace the NaNs with 0
caco2_L1_curated <- caco2_L1_curated %>% 
  mutate(Response = ifelse(is.nan(Response), 0, Response))
```
...
$c_{\textrm{receiver}}$ is defined as the mean R2 response multiplied by its dilution factor. To account for background noise, we also subtract the mean blank response. $V_{\textrm{receiver}}$ is defined by our `Vol.Receiver` column and $\Delta t$ is defined by our `Time` column. 

$c_0$ is calculated similarly to $c_{\textrm{receiver}}$ but with the mean D0 response instead of R2 such that 

$$c_0 = \textrm{dilution factor}_{D0} * \overline{Response}_{D0} - \textrm{dilution factor}_{Blank} * \overline{Response}_{Blank}$$. 

Lastly, $A$ is defined by our `Membrane.Area` column. 

With this information, we can calculate P~app~ from the donor to the receiver compartments in the well. The user can switch the roles of donor and receiver and produce a second set of estimates for P~app~ in the opposite direction. The estimate directions are annotated according to Figure 1. 
...
Our Level 3 data contains estimates for `C0`, `dQdt`, and `Papp` in both directions and a `Refflux` estimate. The Efflux ratio, or the ratio between the two apparent membrane permeabilities is expressed as $R_{\textrm{efflux}} = \frac{P_{app}^{B2A}}{P_{app}^{A2B}}$. 
