...
vars <- c("Parameter", "Default", "Required in L0?", "Corresp. single-entry Parameter", "Descr.")
...
req_cols <- req_cols %>% 
  mutate("Corresp. single-entry Parameter" = ifelse(.data[[vars[[3]]]] == "N", 
...
All of our samples are successfully formatted and returned in `caco2_L1_curated`, the Level 1 data frame. Each sample has one of the following sample types
...
$$c_0 = \textrm{dilution factor}_{D0} *( \overline{Response}_{D0} - \overline{Response}_{Blank})$$. 
...
Our Level 3 data contains estimates for `C0`, `dQdt`, and `Papp` in both directions and a `Refflux` estimate. The Efflux ratio, or the ratio between the two apparent membrane permeabilities can be expressed as $R_{\textrm{efflux}} = \frac{P_{app}^{B2A}}{P_{app}^{A2B}}$. 
