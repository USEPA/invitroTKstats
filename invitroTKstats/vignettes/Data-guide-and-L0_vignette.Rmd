...
The `invitroTKstats` R package is designed to work as a data processing pipeline for *in vitro* assays collecting various physiological parameters used in high-throughput toxicokinetic (HTTK) modeling.  This pipeline is meant to take mass spectrometry (MS) data and estimate the physiological parameters from the relevant *in vitro* assays. These include but are not limited to:
...
- Membrane Permeability via the Caco-2 assay

Here, we discuss the first steps of the data processing pipeline, including the creation of the data guide (a.k.a. data catalog) and compilation of the level-0 (raw) data into a single data.frame. *Note, these steps are meant to be consistent across the various assays/physiological parameter that may be pipelined by this package, i.e. not assay specific. **However, the pipeline is not set up to simultaneously process data from various assays (e.g. $f_{up}$ from RED and $f_{up}$ from UC).***

In this vignette, we are going to use data from the intrinsic hepatic clearance assays (Cl~int~) collected as part of the Kruetz et al. (2023) PFAS manuscript for demonstration purposes.
...
# Raw Excel File Exploration

The first thing we need to do for the pipeling process is to construct a data guide (i.e. data catalog) itemizing which data files and sheets therein contain relevant raw data, as well as other relevant meta-data information. Before creating a data guide with `invitroTKstats` functions, let us first consider the raw data files and their structure in the relevant data directory, called "KreutzPFAS".

*NOTE: The terms "data catalog" and "data guide" are used interchangeably throughout the vignette.*

Since these datasets are not stored as part of the R package it is necessary to clone the repository containing the "KreutzPFAS" sub-directory. The "KreutzPFAS" sub-directory contains all of the raw data files, original data processing scripts, and resulting datasets after processing and parameter estimation. *It should be noted the data pipelining code and resulting datasets were obtained with a prototype version of the `invitroTKstats` package.* In the following code chunk, it will be necessary to replace "<path_to_directory>", in the code chunk below, with the file path to the local repository clone and the "KreutzPFAS" sub-directory.

```{r raw_data_dir,echo=FALSE}
# this code chunk is hidden since it may not be applicable for the R pckg user
raw_data_dir <- "~/Git/invitrotkstats/working/KreutzPFAS/"
```
```{r raw_data_dir_show,eval=FALSE}
# raw data directory - put together the path to where the sub-directory for KreutzPFAS is in your local
raw_data_dir <- "<path_to_directory>/KreutzPFAS/"
```

More than one experiment (i.e. data collection run) is typically necessary to obtain all the relevant data for an analysis.  Thus, there is likely to be more than one Excel file containing raw data that needs to be processed.
...
hep_clint_xlsx <- paste0(raw_data_dir,hep_clint_xlsx)
```
...
Now that the Excel files containing the raw data are identified and we have a general idea of how to programmtically explore the data, we need to create the data guide. The data guide (a.k.a. data catalog) creation step, as previously mentioned, can be thought of as logging all the relevant raw data files for a given physiological parameter we wish to pipeline (i.e. data inventory if you will).  This is a crucial part of the pipeline in that it allows us to identify where all of the raw datafiles are coming from and where we are pulling raw (Level-0) data from within those datasets. Thus, setting up a standard and transparent level-0 data compilation process.  This also provides key meta-data information that can be used for tracking/evaluating data provenance.

Though the data guide (which is ultimately a `data.frame` format) may be created manually through an Excel file, the `invitroTKstats` package includes a function called `create_data_catalog` enabling that automatically generates and documents the data guide creation. Furthermore, this function has built in checks to ensure all the necessary information is provided and in the correct format expected by the pipeline. This allows for maximum reproducibility, transparency, and efficiency in compiling the data guide. Thus, it is highly encouraged and considered best practice to utilize the `create_data_catalog` function for this step.
...
Here, we are going to use the third Excel file (and only the "Data063021" sheet) to demonstrate putting together the data catalog programmatically using the `create_catalog` function within `invitroTKstats`.  It should be noted, the information provided in `create_data_catalog` is obtained by visual inspection of the file of interest and identifying the corresponding input values for each of the arguments.
...
In cases where an data analyst is working with a set of raw data Excel files with a standardized form (typically lab-specific) it may be advantageous to have a wrapper function for streamlining the data guide creation process, such that the function automatically extracts the necessary meta-data to provide for populating the data catalog. This exercise is left up to the reader.

# Chemical ID Map

Once the data catalog is compiled, the only remaining experimental meta-data to collect prior to compiling the comprehensive level-0 data files is the chemical identification mapping table (i.e. chemical ID map). The chemical ID map provides the pipeline with the compound name used by the lab, EPA DSSTox identifier (see [Distributed Structure-Searchable Toxicity (DSSTox) Database](https://www.epa.gov/comptox-tools/distributed-structure-searchable-toxicity-dsstox-database)), and the common compound name.  This typically will be provided by the lab within the Excel files with raw data and/or in a separate "meta-data" file, and this may be lab dependent given the potential for differing lab protocols.
...
- In an ideal scenario, the lab will deliver the relevant data files for processing with the following guidelines:
  - All data sets to be processed together are the the analyst at one time and stored in a common directory location.
  - A standardized filenaming conventions are used to convey the assay type (i.e. physiological parameter), date of generation, and other relevant distinguishing information.
...
- Minimal to no data manipulation in the raw data Excel files should be done by the analyst.  In cases where data manipulation is necessary, it is ideal to programmatically implement the changes (i.e. make changes via coding scripts) to enable transparent and reproducible work, then save the updated file with a "tag" indicating the file has been edited from the originally delivered data.
- Data analysts should generally take the approach of little "data touching", i.e. evaluation of Excel files should largely be programmatic (when possible).  In cases where this is not feasible, no manual changes to the Excel files should be made.
