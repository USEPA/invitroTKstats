...
In this vignette, we are going to use data from the intrinsic hepatic clearance assays (Cl~int~) collected as part of the @kreutz2023category PFAS manuscript for demonstration purposes.
...
- In an ideal scenario, the lab will deliver the relevant data files for processing with the following guidelines:
  ...
  - A standardized filenaming convention is used to convey the assay type (i.e. physiological parameter), date of generation, and other relevant distinguishing information.
...
- Data analysts should generally take the approach of little "data touching", including but not limited to:
  ...
  - Manipulation/editing of raw Excel files should be done programmatically and documented in a code script (e.g. `.R` script) and accompanied by the code script execution output (e.g. `.Rout` file).
  - Raw Excel files manipulated/edited via code should be saved as a separate files wiht a "tag" indicating the file has been edited from the originally delivered data, and any other relevant "tag" information (e.g. date and time of "new data" export).
