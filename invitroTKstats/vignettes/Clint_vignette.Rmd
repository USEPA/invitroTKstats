...
$$ x_{blank,w^*_i} = \frac{\gamma_{w^*_i}}{df_{blank,w^*_i}} $$
...
$$ \tau_{blank,w^*_i} = \frac{1}{(\sigma_{a,w^*_i} + m_{h,w^*_i} *x_{blank,w^*_i})^2} $$
...
where $p$ is the probability parameter and $p_{d_g}$ is the user-specified prior probability of instability for a chemical within the assay (i.e. abiotic degradation) (`DEGRADES.PROB`).

Prior for the slope representing the biological clearance rate (`bio.rate`) for the lower concentration, indicated by the subscript $1$ for $C_{thresh}$ and $conc_{target}$:
...
Prior for the slope representing the abiotic clearance rate, when data from the inactivated hepatocytes is available (`abio.rate`) at the lower concentration, indicated by the subscript $1$ for $C_{thresh}$ and $conc_{target}$:  
...
where $\beta_{abio,i}$ is the JAGS step function for the $i^{th}$ abiotic observation, such that
...
| `cc.obs.conc` | $C_{cc}$ | `Test.Nominal.Conc` indices for `CC` samples |
...
| `DEGRADES.PROB` | $p_{d_g}$ | `degrade.prob` (0.05) | probability of instability for a chemical within the assay (i.e. abiotic degradation) |
