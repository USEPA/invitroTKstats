...
$$ log(\sigma_a)_w \sim Unif(-6,1); \space \sigma_{a,w} = 10^{log(\sigma_a)_w} $$
$$ log(m_h)_w \sim Unif(-6,1); \space m_{h,w} = 10^{log(m_h)_w} $$

$$ C_{thresh,w} \sim Unif(0,\frac{conc_{target,w}}{10})$$

$$ log(cal)_w \sim N(0,0.01); \space cal_w = 10^{log(cal)_w}$$

$$ \gamma_w \sim Exp(100)$$

where $w$ indicates the calibration index and $w = (1,...,n_{cal})$ such that $n_{cal}$ is the total number of mass-spectrometry calibrations.

##### Likelihood for Blank Observations

$$ y_{blank,i} \sim N(x_{blank,w^*_i}= \frac{\gamma_{w^*_i}}{df_{blank,w^*_i}},p_{blank,w^*_i} = \frac{1}{(\sigma_{a,w^*_i} + m_{h,w^*_i} *x_{blank,w^*_i})^2}) $$
where $y_{blank,i}$ is the $i^{th}$ blank observation and $w^*_i$ is the calibration index for that observation, such that $w^* = (w^*_1,...,w^*_{n_{blank}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).

##### Likelihood for Calibration Curve
...
\beta_{cc,i} =
\begin{cases}
1 & if \space (\frac{y_{cc,i}}{df_{cc,i}} - \frac{C_{thresh,w^*_i}}{df_{cc,i}}) \ge 0 \\
0 & o.w. 
\end{cases}
$$

$$ y_{cc,i} \sim N(x_{cc,i} = m_{cc,i} * (\frac{y_{cc,i}}{df_{cc,i}} - \frac{C_{thresh,w^*_i}}{df_{cc,i}}) * \beta_{cc,i} + \frac{\alpha_{cc,i}}{df_{cc,i}},p_{cc,i} = \frac{1}{(\sigma_{a,w^*_i} + m_{h,w^*_i} * x_{cc,i})^2})$$

where $y_{cc,i}$ is the $i^{th}$ calibration-curve observation (sample type = `CC`) and $w^*_i$ is the calibration index for that observation, such that $u^* = (u^*_1,...,u^*_{n_{cc}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).
...
**Slope at the Lower Initial Concentration**:
$$ m_1 = d_c * \delta_{bio} + d_g * \delta_{abio} $$

**Slope at the Higher Initial Concentration**:

$$ m_2 = d_g * \delta_{abio} + d_c * \delta_{bio} *(1-s_f*s_a) $$
**Exponential Decay**:

$$ C_i = conc_{target,c^*_i}*exp(-m_{c^*_i}*t_i) $$
where $m_{c^*_i} \in \{m_1,m_2\}$.

$$
\beta_i = 
\begin{cases}
1 & if \space \frac{C_i}{df_i} - \frac{C_{thresh,c^*_i}}{df_i} \ge 0 \\
0 & o.w.
\end{cases}
$$

$$ y_i \sim N(x_i = cal * (\frac{C_i}{df_i} - \frac{C_{thresh,c^*_i}}{df_i}) * \beta + \frac{\gamma}{df},p_i = \frac{1}{(\sigma_{a,c^*_i} + m_{h,c^*_i}*x_i)^2})$$

where $y_i$ is the $i^{th}$ observation $i = (1,..., n)$ and $c^*_i$ represents the concentration index for the observation $c^* = (c^*_1,...,c^*_n)$.
...
$$ C_{abio,i} = conc_{target,k^*_i} * exp(-m_{abio}*t_{abio,i}) $$
$$
\beta_{abio,i} = 
\begin{cases}
1 & if \space (\frac{C_{abio,i}}{df_{abio,i}} - \frac{C_{thresh,k^*_i}}{df_{abio,i}}) \ge 0 \\
0 & o.w.
\end{cases}
$$

$$ y_{abio,i} \sim N(x_{abio,i} = cal_{k^*_i} * (\frac{C_{abio,i}}{df_{abio,i}} - \frac{C_{thresh,k^*_i}}{df_{abio,i}}) * \beta_{abio,i} + \frac{\gamma_{k^*_i}}{df_{abio,i}},p_{abio,i} = \frac{1}{(\sigma_{a,k^*_i} + m_{h,k^*_i} * x_{abio,i})^2}) $$

where $y_{abio,i}$ is the $i^{th}$ non-biological observation $i = (1,..., n_{abio})$ and $k^*_i$ represents the calibration index for the non-biological observation $k^* = (k^*_1,...,k^*_{n_abio})$.
