...
```{r, echo = FALSE, out.width = "100%", fig.cap = "Fig 1: Cl~int~ experimental set up", fig.topcaption = TRUE, fig.align = "left"}
...
DT::datatable(head(clint_L0, n = 3),
              rownames = FALSE, 
...
Some columns must be present in the Level 0 data while others can be filled with a single value. At minimum, the following columns must be present in the Level 0 data and specified as single entry is not permitted: `sample.col`, `compound.col`, `dtxsid.col`, `lab.compound.col`, `type.col`, `istd.col`, `area.col`, and `note.col`. 
...
req_cols = data.frame(matrix(nrow = 31, ncol = 4))
vars <- c("Parameter", "Required in L0?", "Corresp. single-entry Parameter", "Descr.")
colnames(req_cols) <- vars

# Parameter names 
parameters <- c("FILENAME", "data.in", "sample.col", "date.col", "compound.col",
...
req_cols <- req_cols %>% 
  mutate("Required in L0?" = case_when(
    Parameter %in% c("sample.col","compound.col", "dtxsid.col", "lab.compound.col", "type.col", "istd.col", "area.col", "note.col") ~ "Y",
...
req_cols <- req_cols %>% 
  mutate("Corresp. single-entry Parameter" = ifelse(.data[[vars[[2]]]] == "N", gsub(".col","",Parameter),
                NA))
...
```{r, echo = FALSE, warning = FALSE}
DT::datatable(req_cols,
              caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left;',
    'Table 2: Level 1 parameters'),
    extensions = 'Scroller',
    rownames = FALSE,
...
We receive a warning message that some of our samples have been removed due to "inappropriate sample types". The removed samples include data that did not have one of the following sample types: Blank, Hepatocyte incubation concentration (Cvst), Inactivated Hepatocytes (Inactive), or Calibration Curve (CC). 
...
clint_L0 %>% 
  filter(Type == "QC") %>% 
  head(n = 5) %>% 
  DT::datatable(caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;', 'Table 3: Subset of removed samples'),
                rownames = FALSE,
                extensions = 'Scroller',
                options = list(deferRender = TRUE,
                               scrollX = TRUE, 
...
```{r}
DT::datatable(clint_L4_curated[["Results"]],
              rownames = FALSE,
              caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left;',
