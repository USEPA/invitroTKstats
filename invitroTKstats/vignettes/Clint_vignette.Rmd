...
$$ log(\sigma_a) \sim Unif(-6,1); \space \sigma_a = 10^{log(\sigma_a)} $$
$$ log(m_h) \sim Unif(-6,1); \space m_h = 10^{log(m_h)} $$

$$ C_{thresh} \sim Unif(0,\frac{conc_{target}}{10})$$

$$ log(cal) \sim N(0,0.01); \space cal = 10^{log(cal)}$$

$$ \gamma \sim Exp(100)$$

##### Likelihood for Blank Observations

$$ y_{blank} \sim N(x_{blank}= \frac{\gamma}{df_{blank}},p_{blank} = \frac{1}{(\sigma_a + m_h *x_{blank})^2}) $$
...
\beta_{cc} =
\begin{cases}
1 & if \space (\frac{y_{cc}}{df_{cc}} - \frac{C_{thresh}}{df_{cc}}) \ge 0 \\
0 & o.w. 
\end{cases}
$$


$$ y_{cc} \sim N(x_{cc} = m_{cc} * (\frac{y_{cc}}{df_{cc}} - \frac{C_{thresh}}{df_{cc}}) * \beta_{cc} + \frac{\alpha_{cc}}{df_{cc}},p_{cc} = \frac{1}{(\sigma_a + m_h * x_{cc})^2})$$
...
\beta = 
\begin{cases}
1 & if \space \frac{C}{df} - \frac{C_{thresh}}{df} \ge 0 \\
0 & o.w.
\end{cases}
$$

$$ y \sim N(x = cal * (\frac{C}{df} - \frac{C_{thresh}}{df}) * \beta + \frac{\gamma}{df},p = \frac{1}{(\sigma_a + m_h*x)^2})$$
...
\beta_{abio} = 
\begin{cases}
1 & if \space (\frac{C_{abio}}{df_{abio}} - \frac{C_{thresh}}{df_{abio}}) \ge 0 \\
0 & o.w.
\end{cases}
$$

$$ y_{abio} \sim N(x_{abio} = cal * (\frac{C_{abio}}{df_{abio}} - \frac{C_{thresh}}{df_{abio}}) * \beta_{abio} + \frac{\gamma}{df_{abio}},p_{abio} = \frac{1}{(\sigma_a + m_h * x_{abio})^2}) $$
