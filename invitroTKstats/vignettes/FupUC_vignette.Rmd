...
$$ log(\sigma_a)_w \sim U(-6,1); \space \sigma_{a,w} = 10^{log(\sigma_a)_w} $$

$$ log(m_h)_w \sim U(-6,1); \space m_{h,w} = 10^{log(m_h)_w} $$
...
#### Likelihood for Response Observations
...
$$ y_i \sim N(x_i = m_i*((C_{k^*_i}- C_{thresh,w^*_i}) * \beta_i + \alpha_i), p_i = \frac{1}{(\sigma_{a,w^*_i}+m_{h,w^*_i}*y_i)^2} ) $$
