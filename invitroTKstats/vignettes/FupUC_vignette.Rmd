...
All of our samples are successfully formatted and returned in `fup_uc_L1_curated`, the Level 1 data frame. Each sample has one of the following sample types
...
Our Level 2 data now contains a `Verified` column. If the sample should be included, the column contains a "Y" for yes. If the sample should not be included, the column contains the reason for exclusion. 
...
To run Level 4, one needs to have JAGS installed on their machine. To determine the correct path, the user must run `runjags::findjags()` and include the path up through `/x64`. 
