...
defaults <- c("MYDATA", NA, "Lab.Sample.Name", "Lab.Compound.Name", "DTXSID", 
              ...
              "Note", "Level0.File", "Level0.Sheet", TRUE, FALSE, 5, NA, NA)
...
where $a$ and $b$ are the minimum and maximum, respectively, and $conc_{target}$ is the expected initial concentration (`Test.Nominal.Conc`) for calibration index $w$.
...
Suppose $n$ is the total number of response observations (`Num.obs`), and $y_i$ indicates the $i^{th}$ observation, sample type = "<>" (`<>`). For each observation we obtain a posterior MCMC estimate for the calibration curve observation with the following:

Estimation for the predicted values for response observations (`Response.pred`):

$$ x_i = m_i*((C_{k^*_i}- C_{thresh,w^*_i}) * \beta_i + \alpha_i) $$
...
$$ \beta_i = 
\begin{cases}
1 & if \space C_{k^*_i}- C_{thresh,w^*_i} \ge 0\\
0 & o.w.
\end{cases}
$$

* $w^*_i$ represents the calibration index (`obs.cal`) for the $i^{th}$ observation $w^* = (w^*_1, \ldots, w^*_n)$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the $n_{cal}$ calibrations).


$$  \tau_i = \frac{1}{(\sigma_{a,w^*_i}+m_{h,w^*_i}*y_i)^2} $$

$$ y_i \sim N(\mu = x_i,\tau = \tau_i) $$

where $y_i$ is the $i^{th}$ observation and $w^*_i$ is the calibration index for observation $i$, such that $w^*_i = (w^*_1,...,w^*_n)$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations). Finally, $k^*_i$ is the initial concentration index for observation $i$, such that $k^* = (k^*_1,...,k^*_n)$ and $k*_i \in k$ (i.e. $k^*_i$ indicates one of the expected initial concentrations and $k = (1,...,n_{C})$ is the set of all possible initial concentrations for the experiment, $n_C$ is the total number of nominal concentration values).

#### Binding Model

$$ log(f_{up}) \sim U(-15,0) ; \space f_{up} = 10^{log(f_{up})} $$
$$ log(f_{loss}) \sim U(-6,0) $$
$$ f_{stable} = 1 - 10^{log(f_{loss})} $$

$$ C_i \sim N(conc_{target,w^*_i},100) $$

$$ C_{T5,i} = f_{stable} * C_i $$

$$ C_{AF,i} = f_{up}*C_{T5,i} $$

where $C_i$ is the $i^{th}$ T1 concentration and $w^*_i$ is the calibration index for observation $i$, such that $w^*_i = (w^*_1,...,w^*_n)$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations). $C_{T5,i}$ and $C_{AF,i}$ are the estimated breakdown at T5 and the remaining stable concentration in the aqueous fraction (AF), respectively, given $C_i$.
...
| Response.pred | $x$ | |
| Response.prec | $p$ | |
| Response.obs | $y$ | Normal |
| log.Fup | $log(f_{up})$ | Uniform |
| Fup | $f_{up}$ | |
| log.Floss | $log(f_{loss})$| Uniform |
| Fstable | $f_{stable}$| |
| Conc | $C$ | Normal |
...
\references{
\insertRef{kreutz2023category}
\insertRef{redgrave1975separation}
\insertRef{smeltz2023plasma}
}
