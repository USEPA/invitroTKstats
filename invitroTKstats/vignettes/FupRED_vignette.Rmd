...
Each chemical may have more than one day of experimentation, and thus multiple calibrations.  Suppose for the chemical of interest there are a total of $n_{cal}$ calibrations (`Num.cal`).  For a particular calibration $w \in (1,...,n_{cal})$ we assume the following hyper-priors for our Bayesian model:
...
**Prior for the log-scale hetero-scedastic analytic slope (`log.hetero.analytic.slope`):**
...
#### Estimation of Blanks with No Plasma {#llnpb}

Suppose $n_{NPB}$ is the total number of blank observations without plasma (`Num.NoPlasma.Blank.obs`), and $y_{NPB,i}$ is the $i^{th}$ no plasma blank observation, sample = "" (`NoPlasma.Blank.obs`). For each observation we obtain a posterior MCMC estimate for the observations with:

Estimation for the predicted values for blanks with no plasma (`NoPlasma.Blank.pred`):

$$ x_{NPB,i} = \frac{\gamma_{w^*_i}}{df_{NPB}} $$

where

* $df_{NPB}$ is the dilution factor (`NoPlasma.Blank.df`) for the no plasma blank observations
* $w^*_i$ represents the calibration index (`NoPlasma.Blank.cal`) for the $i^{th}$ observation $w^* = (w^*_1,\ldots,w^*_{n_{NPB}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one fo the $n_{cal}$ calibrations)

Estimation for the precision of blanks with no plasma (`NoPlasma.Blank.prec`):
...
#### Estimation of Blanks with Plasma {#llblanks}

**Prior for estimating the extent of plasma interference (`log.Plasma.Interference`):**

$$ log(\eta_{plasma}) \sim \textrm{Unif} \left( a = -6,b = \frac{log(\frac{10}{conc_{target}})}{log(10)} \right); \space \eta_{plasma} = 10^{log(\eta_{plasma})} $$
...
Estimation for the predictions for blanks with plasma (`Plasma.Blank.pred`):

$$ x_{PB,i} = \frac{cal_{w^*_i} * (\frac{\eta_{plasma}*p_{protein,r^*_i}}{100} - C_{thresh,w^*_i}) * \beta_{PB,i}+\gamma_{w^*_i}}{df_{PB}} $$

where

* $df_{PB}$ is the dilution factor (`Plasma.Blank.df`) for blank observations with plasma
* $p_{protein,r^*_i}$ is the physiological protein percent for the $i^{th}$ plasma blank observation, given the replicate index
* $r^*_i$ is the replicate index for the $i^{th}$ blank observation with plasma (`Plasma.Blank.rep`), such that $r^*_i = (r^*_1,\ldots,r^*_{n_{PB}})$
...
* $w^*_i$ is the calibration index for the $i^{th}$ blank observation with plasma (`Plasma.Blank.cal`), such that $w^* = (w^*_1, ... , w^*_{n_{PB}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).

Estimation for the precision for blanks with plasma (`Plasma.Blank.prec`):

$$ \tau_{PB,i} = \frac{1}{(\sigma_{a,w^*_i}+m_{h,w^*_i}*x_i)^2} $$

**Likelihood for the plasma blank observations (`Plasma.Blank.obs`):**

$$ y_{PB,i} \sim \textrm{N} \left( \mu = x_{PB,i},\tau = \tau_{PB,i} \right) $$

#### Estimation for T0 {#llt0}

Suppose $n_{T0}$ is the total number of T0 observations (`Num.T0.obs`), and $y_{T0,i}$ indicates the $i^{th}$ T0 observation, sample type = "T0" (`T0.obs`). For each observation we obtain a posterior MCMC estimate for the initial time observations with the following:

Estimation for the predictions for the T0 observations (`T0.pred`):

$$ x_{T0,i} = \frac{cal_{w^*_i}*(conc_{target}-\eta_{plasma}-C_{thresh,w^*_i})*\beta_{T0,i}+\gamma_{T0,w^*_i}}{df_{T0}} $$
...
* $w^*_i$ is the calibration index for that observation, such that $w^* = (w^*_1, ... , w^*_{n_{T0}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).
...
$$ y_{T0,i} = N(\mu = x_{T0,i},\tau = \tau_{T0,i}) $$

#### Estimation of the Calibration Curve {#llcc}
...
$$
x_{CC,i} = \frac{cal_{w^*_i}*(C_{CC,i} - \eta_{plasma} - C_{tresh,w^*_i})*\beta_{CC,i}+cal{w^*_i}*\eta_{plasma}*\gamma_{w^*_i}}{df_{CC}}
$$
...
$$ \beta_{CC,i} =
\begin{cases}
1 & \textrm{if} \space (C_{CC,i} - \eta_{plasma} - C_{tresh,w^*_i}) \ge 0 \\
0 & \textrm{otherwise}
\end{cases}
$$

* $w^*_i$ is the calibration index for that observation, such that $w^*=(w^*_1, ... , w^*_{n_{CC}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).
...
$$ log(K_d) \sim \textrm{Unif} \left( -10,5 \right) ; \space K_d = 10^{log(K_d)} $$

$$ f_{up} = \frac{K_d}{K_d + \rho_{plasma}} $$

##### Concentrations for Replicates

Estimation for the protein concentration for an observation (`C.protein`):

$$ C_{protein,i} = \frac{\rho_{plasma}*p_{protein,i}}{100}  $$
...
$$ C_{missing,i} \sim \textrm{Unif} \left( 0,conc_{target} \right) $$ 

Estimation of the concentration of the chemical unbound in both wells (`C.u`):

$$ C_{u,i} = (conc_{target}-C_{missing,i}) * \frac{K_d}{2*K_d+C_{protein,i}} $$

Estimation of the concentration of the chemical bound in the plasma well (`C.b`):

$$ C_{b,i} = \frac{(conc_{target}-C_{missing,i})*C_{protein,i}}{2*K_d+C_{protein,i}} $$

Estimation of the total chemical concentration in the plasma well (`C.total`):

$$ C_{total,i} = C_{b,i} + C_{u,i} $$

#### Estimation of PBS {#llpbs}

Estimation of the chemical concentration for the $i^{th}$ PBS observation ($C_{PBS,i}$), assuming the unbound chemical concentration for the corresponding replicate index ($C_{u,r^*_i}$):
...
$$
x_{PBS,i} = \frac{cal_{w^*_i}*(C_{PBS,i} - C_{thresh,w^*_i})*\beta_{C_{PBS},i}+\gamma_{w^*_i}}{df_{PBS}}
$$
...
* $y_{PBS,i}$ is the $i^{th}$ PBS observation with no plasma
* $r^*_i$ is the replicate index for the $i^{th}$ PBS observation
* $\beta_{C_{PBS},i}$ is the JAGS step function for the $i^{th}$ PBS observation, such that
...
* $w^*_i$ is the calibration index for that observation, such that $w^* = (w^*_1, ... , w^*_{n_{PBS}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).

#### Estimation for Plasma {#llplasma}

Estimation of the plasma concentration ($C_{plasma}$), assuming the total concentration for the corresponding calibration index ($C_{total,w^*_i}$):

$$ C_{plasma} = C_{total,w^*_i} $$

Estimation for the plasma predictions (`Plasma.pred`):

$$
x_{plasma,i} = \frac{cal_{w^*_i}*(C_{plasma,i} - \frac{\eta_{plasma}*\tau_{plasma,r^*_i}}{100} - C_{thresh,w^*_i})*\beta_{plasma,i} + cal_{w^*_i} * \frac{\eta_{plasma}*\tau_{plasma,r^*_i}}{100} + \gamma_{w^*_i}}{df_{plasma}}
$$
...
* $y_{plasma,i}$ is the $i^{th}$ plasma observation
* $r^*_i$ is the plasma replicate for the $i^{th}$ observation
* $w^*_i$ is the calibration index for the $i^{th}$ observation.
...
| Num.cal | | |
| Physiological.Protein.Conc | $\rho_{protein}$ | |
| Assay.Protein.Percent | $p_{protein}$ | |
| Num.Plasma.Blank.obs | $n_{PB}$ | |
| Plasma.Blank.obs | $y_{PB}$ | |
| Plasma.Blank.cal | | |
| Plasma.Blank.df | $df_{PB}$ | |
| Plasma.Blank.rep | | |
...
| calibration | $cal$ | |
| NoPlasma.Blank.pred | $x_{NPB}$ | |
| NoPlasma.Blank.prec | $\tau_{NPB}$ | |
| NoPlasma.Blank.obs | $y_{NPB}$ | Normal |
| log.Plasma.Interference | $log(\eta_{plasma})$ | Uniform |
| Plasma.Interference | $\eta_{plasma}$ | |
| Plasma.Blank.pred | $x_{PB}$ | |
| Plasma.Blank.prec | $\tau_{PB}$ | |
| Plasma.Blank.obs | $y_{PB}$ | Normal |
| T0.pred | $x_{T0}$ | |
| T0.prec | $\tau_{T0}$ | |
| T0.obs | $y_{T0}$ | Normal |
| CC.pred | $x_{CC}$ | |
| CC.prec | $\tau_{CC}$ | |
| CC.obs | $y_{CC}$ | Normal |
| log.Kd | $log(K_d)$ | Uniform |
| Kd | $K_d$ | |
| Fup | $f_{up}$ | |
| C.protein | $C_{protein}$ | |
| C.missing | $C_{missing}$ | |
| C.u | $C_u$ | |
| C.b | $C_b$ | |
| C.total | $C_{total}$ | |
| PBS.conc | $C_{PBS}$ | |
| PBS.pred | $x_{PBS}$ | |
| PBS.prec | $\tau_{PBS}$ | |
| PBS.obs | $y_{PBS}$ | Normal |
| Plasma.conc | $C_{plasma}$ | |
| Plasma.pred | $x_{plasma}$ | |
| Plasma.prec | $\tau_{plasma}$ | |
| Plasma.obs | $y_{plasma}$ | Normal |
