...
```{r L1 processing}
# Rename EC1 as EQ1 AND EC2 as EQ2 so they do not get removed 
fup_red_L0_curated <- fup_red_L0 %>% 
...
6. Equilibrium Control Well 1 (EC1)
7. Equilibrium Control Well 2 (EC2)
...
Mathematically, f~up~ is the ratio of the compound concentration in the buffer side to the plasma side. However, in some experiments, the phosphate buffered solution (PBS) is loaded into both sides of the chamber; the difference between the sides is then differentiated by which side contains the plasma. Therefore, to increase clarity, we rename the buffer side to the acceptor well and the plasma side to the donor well such that $$f_{up} = \frac{C_{\textrm{acceptor}}}{C_{\textrm{donor}}}$$ where $C_{\textrm{acceptor}}$ is the compound concentration in the acceptor well and $C_{\textrm{donor}}$ is the compound concentration in the donor well. 
...
We pass `fup_red_L2_curated`, the Level 2 data frame, and **not** `fup_red_L3_curated`, the Level 3 data frame, into `calc_fup_red`. This is because Level 3 and Level 4 processing are not sequential; they are methods that calculate different statistical quantities. 
