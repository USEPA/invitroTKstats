...
Some of the indexes are reused between sections (e.g. $i$, $w^*_i$, etc.).  However, it should be noted that these are not meant to be understood across sub-sections; rather, only understood within the context of the section they are in.
...
Each chemical may have more than one day of experimentation, and thus multiple calibrations.  Suppose for the chemical of interest there are a total of $n_{cal}$ calibrations.  For a particular calibration $w \in (1,...,n_{cal})$ we assume the following hyper-priors for our Bayesian model:
...
where $\mu$ and $\tau$ are the mean and precision, respectively.
...
where $y_{NPB,i}$ is the $i^{th}$ blank observations with no plasma and $w^*_i$ is the calibration index for that observation, such that $w^* = (w^*_1, ... , w^*_{n_{NPB}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).

#### Estimation of Blanks with Plasma {#llblanks}

**Prior for estimating the extent of plasma interference (`log.Plasma.Interference`):**

$$ log(\eta_{plasma}) \sim \textrm{Unif} \left( a = -6,b = \frac{log(\frac{10}{conc_{target}})}{10} \right); \space \eta_{plasma} = 10^{log(\eta_{plasma})} $$

Estimation of the predictions for blanks with plasma (`Plasma.Blank.pred`):

$$ x_{PB,i} = \frac{cal_{w^*_i} * (\frac{\eta_{plasma}*p_{protein,r^*_i}}{100} - C_{thresh,w^*_i}) * \beta_{PB,i}+\gamma_{w^*_i}}{df_{PB}} $$

Estimation of the precision for blanks with plasma (`Plasma.Blank.prec`):
...
* $y_{PB,i}$ is the $i^{th}$ blank observation with plasma
* $r^*_i$ is the replicate index for the $i^{th}$ blank observation with plasma
...
* $w^*_i$ is the calibration index for the $i^{th}$ blank observation, such that $w^* = (w^*_1, ... , w^*_{n_{PB}})$ and $w^*_i \in w$ (i.e. $w^*_i$ indicates one of the calibrations).

#### Estimation for T0 {#llt0}

Estimation of the predictions for the T0 observations (`T0.pred`):
...
* $y_{T0,i}$ is the $i^{th}$ T0 observation
...
* $y_{CC,i}$ is the $i^{th}$ calibration curve observation
