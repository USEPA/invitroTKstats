...
$$ x_{T0,i} = \frac{cal_{w^*_i}*(conc_{target}-\eta_{plasma}-C_{thresh,w^*_i})*\beta_{T0,i}+\gamma_{w^*_i}}{df_{T0}} $$
...
$$
x_{CC,i} = \frac{cal_{w^*_i}*(C_{CC,i} - \eta_{plasma} - C_{tresh,w^*_i})*\beta_{CC,i}+cal_{w^*_i}*\eta_{plasma} + \gamma_{w^*_i}}{df_{CC}}
$$
...
where $\rho_{protein}$ is the assumed physiological protein concentration available for binding (`Physiological.Protein.Conc`).
...
where $\rho_{protein}$ is the assumed physiological protein concentration available for binding and $p_{protein,i}$ is the physiological protein percent for the $i^{th}$ replicate group.
...
* $p_{protein,r^*_i}$ is the physiological protein percent for the $i^{th}$ plasma observation, givne the replicate index
...
| Assay.Protein.Percent | $p_{protein}$ | the physiological protein percent for each `Plasma` sample type replicate group |
| Num.Plasma.Blank.obs | $n_{PB}$ | total number of `Plasma.Blank` samples |
| Plasma.Blank.obs | $y_{PB}$ | the `Plasma.Blank` sample responses |
| Plasma.Blank.cal | $w^*_i$ | calibration index for `Plasma.Blank` samples (reused index) |
...
|-----------------|----------|---------------------------|-------------|
| Physiological.Protein.Conc | $\rho_{protein}$ | `Physiological.Protein.Conc` (1052.632 = 70/(66.5 * 1000) * 1e+06) | the assumed physiological protein concentration available for binding |
