...
$$ C_{thresh,w} \sim U(0,\frac{conc_{target,w}}{10}) $$
$$ log(cal)_w \sim N(0,0.01) ; \space cal_w = 10^{log(cal)_w} $$

$$ \gamma_w \sim Exp(100) $$

#### Likelihood for Blanks with No Plasma

$$ y_{NPB,i} \sim N(x_{NPB,i} = \frac{\gamma_{w^*_i}}{df_{NPB}},p_{NPB,i} = \frac{1}{(\sigma_{a,w^*_i}+m_{h,w^*_i}*x_{NPB,i})^2})$$
...
#### Likelihood for Blanks with Plasma

$$ log(\eta_{plasma}) \sim U(-6,\frac{log(10/conc_{target})}{10}); \space \eta_{plasma} = 10^{log(\eta_{plasma})} $$

$$ \beta_{PB,i} = 
\begin{cases}
1 & if \space (\frac{\eta_{plasma}*p_{protein,r^{PB}_i}}{100} - C_{thresh,w^*_i}) \ge 0 \\
0 & o.w
\end{cases}
$$ 

$$ y_{PB,i} \sim N(x_{PB,i} = \frac{cal_{w^*_i} * (\frac{\eta_{plasma}*p_{protein,r^{PB}_i}}{100} - C_{thresh,w^*_i}) * \beta_{PB,i}+\gamma_{w^*_i}}{df_{PB}},p_{PB,i} = \frac{1}{(\sigma_{a,w^*_i}+m_{h,w^*_i}*x_i)^2}) $$

#### Likelihood for T0

$$ \beta_{T0,i} = 
\begin{cases}
1 & if \space (conc_{target}-\eta_{plasma}-C_{thresh,w^*_i}) \ge 0 \\
0 & o.w. 
\end{cases}
$$ 

$$ y_{T0,i} = N(x_{T0,i} = \frac{cal_{w^*_i}*(conc_{target}-\eta_{plasma}-C_{thresh,w^*_i})*\beta_{T0,i}+\gamma_{T0,w^*_i}}{df_{T0}},p_{T0,i} = \frac{1}{(\sigma_{a,w^*_i}*m_{h,w^*_i}*x_{T0,i})^2}) $$
...
|---------------------|-----------|--------------|
| log.const.analytic.sd | $log(\sigma_a)$ | |
| log.hetero.analytic.slope | $log(m_h)$ | |
| C.thresh | $C_{thresh}$ | |
| log.calibration | $log(cal)$ | |
| background | $\gamma$ | |
| const.analytic.sd | $\sigma_a$ | |
| hetero.analytic.slope | $m_h$ | |
| calibration | $cal$ | |
| NoPlasma.Blank.pred | $x_{NPB}$ | |
| NoPlasma.Blank.prec | $p_{NPB}$ | |
| NoPlasma.Blank.obs | $y_{NPB}$ | |
| log.Plasma.Interference | $log(\eta_{plasma})$ | |
| Plasma.Interference | $\eta_{plasma}$ | |
| Plasma.Blank.pred | $x_{PB}$ | |
| Plasma.Blank.prec | $p_{PB}$ | |
